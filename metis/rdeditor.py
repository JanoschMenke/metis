...
from utils import ptable_widget
from rdkit import Chem


# The main window class
class rdeditor(QtWidgets.QMainWindow):
    # Constructor function
    def __init__(self, fileName=None, loglevel="WARNING"):
        super(rdeditor, self).__init__()
        self.pixmappath = (
            os.path.abspath(os.path.dirname(__file__)) + "/design/pixmaps/"
        )
        self.loglevels = ["Critical", "Error", "Warning", "Info", "Debug", "Notset"]
        self.editor = molEditWidget.MolEditWidget()
        self.editor._action = "Add"
        self.ptable = ptable_widget.PTable()
        self._fileName = None
    ...
    def saveFile(self):
        try:
            Chem.MolToSmiles(self.editor.mol)
            self.statusBar().showMessage("Saved Molecule")
        except:
            self.statusBar().showMessage("Invalid Molecule!")

    def resetMol(self):
        """Set the Molecule back to the orignal molecule"""
        self.editor.mol = self.editor.originalMol
        self.statusBar().showMessage("Reset Molecule")
    ...
    def exitFile(self):
        ...
        if response == "Y":
            self.ptable.close()
            self.hide()  # TODO, how to exit qapplication from within class instance?
        else:
