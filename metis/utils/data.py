...
# The `evalDataFrame` class is a subclass of `pd.DataFrame` that adds additional functionality for
# evaluating and manipulating data, including storing settings, calculating ratings, and managing atom
# selections.
class evalDataFrame(pd.DataFrame):
    def __init__(self, data, settings: Dict):
        super().__init__(data=data)
        self.settings = settings
        self.globalLiab = settings["ui"]["global_properties"]["liabilities"]
        self.numColsBefore = self.shape[0]
        self.liability_ll = [
        ...
        if settings["ui"]["tab"]["render"] == True:
            ...
            tab_names = ["General"]
        for i in self.liability_ll:
            for name in tab_names:
                self[f"{i}AtomSelection_{name}"] = np.empty((len(self), 0)).tolist()
                self[f"{i}SMARTS_{name}"] = ""
        for name in tab_names:
            self[f"otherTextEntry_{name}"] = ""

        if settings["ui"]["general"]["slider"] == True:
            self["rating"] = 50
        else:
            self["rating"] = None
        self.loc[:, self.globalLiab] = 0
        self["alternativeMolecule"] = ""
        self["evaluated"] = 0

    def getPropertyLabels(self, index: int):
        out_dict = {
            name: self[self.settings["propertyLabels"][name]].iloc[index]
            for name in self.settings["propertyLabels"]
        }
        return out_dict
    ...
    def getGlobalLiab(self, index: int):
        return self.loc[index, self.globalLiab].values.tolist()
...
def loadData(
    settings: Dict, initial: bool = True, model=None
    ...
    if initial:
        print(settings["data"]["initial_path"])
    ...
    if settings["data"]["selection_strategy"] == "diverse":
        selector = randomActiveSelector(
            keep_df, settings["activity_label"], settings["data"]["num_molecules"]
        )

    if settings["data"]["selection_strategy"] == "epig":
        assert model is not None
        selector = epigActiveSelector(
            keep_df,
            settings["activity_label"],
            model,
            settings["data"]["num_molecules"],
        )
    ...
    return df, df.iloc[0, -(df.shape[1] - df.numColsBefore) : -1]
...
class epigActiveSelector:
    ...
    def get_selection(self) -> List[str]:
        ...
        `picker` object.
        """
        smiles_pool = self.df[self.df[self.activity_label] > 0.5]
        if len(smiles_pool) > 1000:
            smiles_pool = smiles_pool.sample(
                n=1000,
                replace=False,
            ).SMILES.tolist()
        else:
            smiles_pool = smiles_pool.SMILES.tolist()
        # target input distribution (top 1000 highest scoring molecules)
        smiles_target = (
            self.df.sort_values(by=[self.activity_label], ascending=False)
            .head(1000)
            .SMILES.tolist()
        )
        pool_dist = self.model.get_prob_distribution(smiles_pool)
        target_dist = self.model.get_prob_distribution(smiles_target)
        estimated_epig_scores = self.model.estimate_epig(pool_dist, target_dist)
        idx_to_select = list(
            np.argsort(estimated_epig_scores)[::-1][: self.num_to_select]
        )
        return idx_to_select
...
class qbcActiveSelector:
    ...
    def get_selection(self) -> List[str]:
        ...
        `picker` object.
        """
        print(self.df[self.df[self.activity_label] > 0.5].shape)
