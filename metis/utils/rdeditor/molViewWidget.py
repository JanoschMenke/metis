...
class MolWidget(QtSvg.QSvgWidget):
    def __init__(self, mol=None, parent=None):
        ...
        self._valid = None  # check if molecule is valid
    ...
    def getMolSvg(self):
        ...
        if self._drawmol != None:
            ...
            if not self._valid:
                opts.setBackgroundColour((1, 0.48, 0.46))
            for tag in chiraltags:
                idx = tag[0]
                opts.atomLabels[idx] = (
                    self._drawmol.GetAtomWithIdx(idx).GetSymbol() + ":" + tag[1]
                )
            if (len(self._selectedAtoms) > 0) and (  # chcek if atoms are selected
                # when clicking next the old molecule is draw ones before switching,
                # this can cause problems when the idx of the selected atom of the new molecule
                # do not exist in the current molecule, the second condition will catch that
                # and prevent the drawing of the old molecule
                max(self._selectedAtoms)
                <= self._drawmol.GetNumAtoms()
            ):
                if self.drawColor == "global":
                    colors = {}
                else:
                    colors = {
                        self._selectedAtoms[i]: self.colorDict[self.drawColor]
