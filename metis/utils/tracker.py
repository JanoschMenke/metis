...
class metis_tracker:
    ...
    def save_substruct(self, path: str):
        out_dict = {}
        print(out_dict, self.substructure_dict)
    ...
    def get_smiles(self, dataframe: pd.DataFrame):
        self.smiles_dict["desired"] += dataframe[
            dataframe.rating == 2
        ].SMILES.values.tolist()
        self.smiles_dict["alerts"] += dataframe[
            dataframe.rating == 0
        ].SMILES.values.tolist()
        self.smiles_dict["somewhat"] += dataframe[
            dataframe.rating == 1
        ].SMILES.values.tolist()
    ...
    def process_molecular_size(self, df: pd.DataFrame) -> None:
        size_liked_mol = [
            Chem.MolFromSmiles(x).GetNumAtoms() for x in df[df.rating == 2].SMILES
        ]
